...
//  BuyNow
//
//  Created by Sachit Mittal on 23/09/25.
//
...
struct BuyNowView: View {
    ...
    init(price: Double, discountPercentage: Int?, discountColor: Color = .blue, bgColor: Color = .red, yOffset: Double = -20, discountAngle: Double = -10) {
        self.price = price
...
#Preview {
    ZStack{
        ...
        VStack{
            BuyNowView(
                price: 20, discountPercentage: 75
            )
            
            BuyNowView(
                price: 20, discountPercentage: nil
            )
        }
    }
}
